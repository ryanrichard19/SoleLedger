...
public static class UsersApi
{
    public static RouteGroupBuilder MapUsers(this IEndpointRouteBuilder routes)
    {
        ...
        group.MapIdentityApi<SoleLedgerUser>();

        // The MapIdentityApi<T> doesn't expose an external login endpoint so we write this custom endpoint that follows
        // a similar pattern
        group.MapPost("/token/{provider}", async Task<Results<SignInHttpResult, ValidationProblem>> (string provider, ExternalUserInfo userInfo, UserManager<SoleLedgerUser> userManager, SignInManager<SoleLedgerUser> signInManager) =>
        {
            ...
            if (user is null)
            {
                user = new SoleLedgerUser() { UserName = userInfo.Username };
